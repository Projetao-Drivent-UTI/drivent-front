...
export default function Activies() {
  ...
  return (
    <Container>
      ...
      <DayBox>
        { dates?.map((item) =>
          <DateButton onClick={() => setSelectedDate(item) } isSelected={selectedDate===item?true:false}>{dayjs(item, 'YYYY-MM-DD HH:mm:ss.SSS').format('dddd, DD-MM', 'pt-br', { locale: 'pt-br' })}</DateButton>
        )}
      </DayBox>
      {dates.length!==0 ? (selectedDate!==''?
        <ActiviesList date = {selectedDate}></ActiviesList>: <></>) : <></>
      }
    </Container>
  );
}
